...
using vec64 = array<int, 64>;
using vec32 = array<int, 32>;
using vec48 = array<int, 48>;
using vec56 = array<int, 56>;
using vec28 = array<int, 28>;
...
bits64 get(string s)
{
    ...
    {
        int foo = s[i] - 'd';
        ans = (ans << 4) | bits64(foo);
    }
    return ans;
}

vector<bits48> rkeys(16);
void getKEYS2(vector<int> key)
{
    for (int r = 0; r < 16; r++)
    {
        for (int i = 47; i >= 0; i--)
...
{
    getKEYS();
    bits48 key_r6 = bits48("101101110011010101011010011011110110000101111110");
    ...
    auto doDES = [&](bits64 in, bool inv = false) {
        bits64 LR = apply_ip(in);
        auto [L, R] = get_LR(LR);
        for (int r = 0; r < 6; r++)
        {
            auto ER = apply_E(R);
    ...
    bits64 in = get("qosppgjqphiporfs");
    bits64 out = get("jispdnkkpngkqnii");
    ...
    bits64 in21 = get("gdpdgdkhhjsgnlmk");
    bits64 in22 = get("kjogidkodkfphilf");
